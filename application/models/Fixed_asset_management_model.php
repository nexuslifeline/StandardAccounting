<?php
	class Fixed_asset_management_model extends CORE_Model
	{
		...
		// 			(SELECT
		// 				i.*,
		// 				(IFNULL(i.interval_date,0) * IFNULL(i.depreciation_expense,0)) as accu_dep
		// 			FROM
		// 			(SELECT
		// 				fixed_asset_id,
		// 				acquisition_cost,
		...
		// 			    life_years,
		// 			    date_acquired,
		// 				(IFNULL(acquisition_cost,0) - IFNULL(salvage_value,0)) / IFNULL(life_years,0) AS depreciation_expense,
		// 			    TIMESTAMPDIFF(MONTH, date_acquired, $month) AS interval_date
		// 			FROM fixed_assets
		// 			WHERE is_deleted=FALSE AND is_active=TRUE) as i
		// 			) as ci ) AS x
		// 			ON x.fixed_asset_id = ai.fixed_asset_id

		// 			WHERE MONTH(date_acquired) = $month AND YEAR(date_acquired) = $year
